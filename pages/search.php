...
<a href="home.html"  style="color:black;font-weight:bold;float:right;font-size:20px;">&times;</a>
   ...
   <div class="av">
     ...
     <?php
     $details = mysqli_query($conn,"select * from address where city='$search' or pin='$search'");
     $ap = mysqli_query($conn,"select * from houseinfo where status='unsold'");
       $count = mysqli_num_rows($ap);
     require "sell.php";
   ...
   while($dat = mysqli_fetch_array($details))
   {
     ...
     $ad = mysqli_fetch_array(mysqli_query($conn,"select * from houseinfo where email='$dat[email]' AND status='unsold'"));
     $DBIMG->show($ad['img_name']);
     $val=$ad['img_name'];
  echo "<b>Sell/Rent:</b> $ad[sellrent] <br><b>Pricing(INR):</b> $ad[price] <br><b>Description:</b> $ad[hd] <br><b>Location:</b> $dat[address] <br> $dat[city]  $dat[pin] <br><b>Owner:</b> $ad[name]  <b>Phone:</b> $ad[phone] <br><b>Email:</b> $ad[email]<br>";
